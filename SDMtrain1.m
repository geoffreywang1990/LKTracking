...
images = cell(m,1);
currentConfurgurations = zeros(4,m*n*5);
models = cell(5,1);
for u = 1:m
    ...
    currentConfurgurations(:,(u-1)*5*n+1:u*5*n) = temp;
end

fprintf('start created models\n')

for i = 1:5
    D = genDisplacementMatrix(currentConfurgurations,frameAnnotations,m,n);
    F = genFeatureMatrix(images,currentConfurgurations,m,n);
    [W,currentConfurgurations] = learnMappingAndUpdateConfigurations(F,D,currentConfurgurations);
